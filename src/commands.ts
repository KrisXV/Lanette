...
const commands: Dict<ICommandDefinition> = {
	...
	emoji: {
		command(target, room, user) {
			const emojiDatabase = Storage.getGlobalDatabase().emojis;
			if (!emojiDatabase) return;
			if (!target) return;
			if (this.isPm(room)) {
				if (!Users.self.canPerform(Rooms.get('ruinsofalph') as Room, 'bot')) return;
	...
	emojislist: {
		command(target, room, user) {
			...
			buf += `</ul></details>`;
			buf += `<br /><p>`;
			if (room.id !== 'staff') buf += `To use these, you need to be Room Owner (#) or higher in the room it is used in or otherwise whitelisted by the Room Owners. Room Owners can whitelist users in their room with <code>@ewl roomadd/roomremove [userid]</code>. `;
